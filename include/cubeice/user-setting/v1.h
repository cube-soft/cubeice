 ...
 */
/* ------------------------------------------------------------------------- */
#ifndef CUBEICE_USER_SETTING_V1_H
#define CUBEICE_USER_SETTING_V1_H
...
///
/// <summary>
/// 以下は cubeice-ctxdata.h に記述されていたものです。
/// 現在のところ、旧バージョンのデータ構造から変換する際に使用しています。
/// </summary>
//
/* ------------------------------------------------------------------------- */
...
#ifndef IDB_COMPRESSION
#define IDB_COMPRESSION         102
#endif // IDB_COMPRESSION

#ifndef IDB_DECOMPRESSION
#define IDB_DECOMPRESSION       103
#endif // IDB_DECOMPRESSION
...
struct SUB_MENU_ITEM {
	std::size_t             dispSetting;    // Display setting
...
const SUB_MENU_ITEM MenuItem[] = {
	{ COMPRESS_FLAG,	IDB_COMPRESSION,	MAKE_AWSTRING( "圧縮" ),				SubMenuCompress,	MAKE_AWSTRING( "圧縮" ),			NULL },
	{ DECOMPRESS_FLAG,	IDB_DECOMPRESSION,	MAKE_AWSTRING( "解凍" ),				SubMenuDecompress,	MAKE_AWSTRING( "解凍" ),			NULL },
	{ MAIL_FLAG,		IDB_COMPRESSION,	MAKE_AWSTRING( "圧縮してメール送信" ),	SubMenuCompAndMail,	MAKE_AWSTRING( "圧縮してメール送信" ),	NULL },
	{ 0, ICON_NOT_USED, NULL, NULL, NULL, NULL, NULL, NULL } // end marker
};

#endif // end of cubeice-ctxdata.h
...
namespace v1 {
	...
	public:
		...
		user_setting(const bool &delay = false) :
			...
			ctx_flags_(0x03), sc_flags_(0), sc_index_(0), filters_(), update_(true), loglevel_(50), associate_invoke_(false), changed_(false) {
			...
			if (!lResult) {
				...
				if (!PathFileExists(xmlpath)) {
					...
					try {
						...
						boost::property_tree::read_xml(Encode(xmlpath), root);
				...
				if (RegQueryValueEx(hkResult, CUBEICE_REG_FILTER, NULL, &dwType, (LPBYTE)buffer, &dwSize) == ERROR_SUCCESS) {
					...
					filters_.clear();
					boost::algorithm::split(filters_, s, boost::is_any_of(_T("<>")));
				}
		...
		void save() {
			...
			if (!lResult) {
				...
				try {
					...
					write_xml(Encode(xmlpath), root, std::locale(), xml_writer_make_settings(' ', 2, widen<char>("utf-8")));
				}
				catch (...) {
				...
				string_type dest = boost::algorithm::join(filters_, _T("<>"));
		...
		bool require_admin() const { return associate_invoke_; }
...
cleanup:
		...
		void context_read(const boost::property_tree::ptree &pt, std::vector<SUBMENU> &parent)
		{
			...
			BOOST_FOREACH(const ptree::value_type &v, pt) {
				if(v.first == "item") {
					...
					if(attr_id) {
#ifdef	UNICODE
						parent.push_back(SUBMENU( *attr_id, attr_name ? CubeICE::Utf8ToUnicode(*attr_name) : L"" ));
#else
						parent.push_back(SUBMENU( *attr_id, attr_name ? cubeice::Utf8ToSjis(*attr_name) : "" ));
#endif
		...
		void context_write(boost::property_tree::ptree &pt, const std::vector<SUBMENU> &submenu)
		{
			...
			BOOST_FOREACH(const SUBMENU &s, submenu) {
				...
				if(s.str!=_T("")) {
#ifdef	UNICODE
					attr.put("name", cubeice::UnicodeToUtf8(s.str));
#else
					attr.put("name", cubeice::SjisToUtf8(s.str));
#endif
				}
		...
		///
		/// <summary>
		/// 文字エンコーディングを UTF-8 に変換します。引数に指定される
		/// 文字列の文字エンコーディングは、マルチバイト文字設定の場合
		/// は ASCII (CP932)、ユニコード文字設定の場合は Unicode と
		/// となります。
		/// </summary>
		///
		/* ----------------------------------------------------------------- */
		static std::basic_string<char> Encode(const string_type& src) {
#if defined(UNICODE) || defined(_UNICODE)
				return PsdotNet::Encoding::Convert<PsdotNet::Encoding::UTF8>(src);
#else
				return PsdotNet::Encoding::Convert<PsdotNet::Encoding::UTF8>(src, PsdotNet::Encoding::ASCII);
#endif
		}
